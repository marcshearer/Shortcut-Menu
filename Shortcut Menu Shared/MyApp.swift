...
class MyApp {
    ...
    public func start() {
        ...
        Utility.executeAfter(delay: 10) {
            Backup.shared.backup()/*.restore(dateString: "2024-12-13-16-20-55-127")*/ ; self.sound() ; Utility.executeAfter(delay: 1.5) { self.sound() ; Utility.executeAfter(delay: 1.5) { self.sound() }}
        }
